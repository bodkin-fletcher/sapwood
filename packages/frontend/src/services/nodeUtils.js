...
// Simulate a heartbeat check for a node
export const performHeartbeat = async (node) => {
  try {
    // In a real app, we would make an actual HTTP request to the node
    // For this demo, we'll simulate network behavior
    
    // Random success rate based on current status
    const successRate = node.status === 'active' ? 0.9 : 0.3;
    ...
    return {
      success: false,
      timestamp: new Date().toISOString(),
      message: error.message || 'An error occurred during heartbeat check'
    };
  }
};
...
// Execute an API call to a node
export const executeNodeApiCall = async (node) => {
  try {
