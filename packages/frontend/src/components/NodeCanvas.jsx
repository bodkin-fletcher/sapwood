...
const NodeCanvas = ({ nodes, onSelectNode, selectedNodeId, onSelectConnection }) => {
  ...
  // Heartbeat interval
  useEffect(() => {
    // Only run if monitoring is enabled and heartbeat interval is valid
    if (!settings.monitoringEnabled || !settings.heartbeatInterval || settings.heartbeatInterval < 1) {
      return;
    }
    ...
    const intervalId = setInterval(async () => {
      ...
      for (const node of nodes) {
        ...
        try {
          // Perform heartbeat check with retry settings
  ...
  }, [nodes, settings.heartbeatInterval, settings.monitoringEnabled, settings.retryAttempts, settings.retryDelay, heartbeatInProgress, updateNode]);
